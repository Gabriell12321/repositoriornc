...
def extract_all_indicators_data():
    ...
    try:
        ...
        output_file = data_dir / 'complete_indicators_data.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(dashboard_data, f, indent=2, ensure_ascii=False, default=str)

        print(f'\n✅ Dados completos salvos em: {output_file.relative_to(root)}')
...
def create_indicators_tab_structure(dashboard_data):
    ...
    # Salvar estrutura da aba em data/
    root = Path(__file__).resolve().parents[2]
    data_dir = root / 'data'
    data_dir.mkdir(parents=True, exist_ok=True)
    out_path = data_dir / 'indicators_tab_structure.json'
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump(tab_structure, f, indent=2, ensure_ascii=False, default=str)

    print(f'✅ Estrutura da aba Indicadores salva em: {out_path.relative_to(root)}')
