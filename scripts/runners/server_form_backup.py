...
import logging
import sys
from flask import Flask, request, jsonify, send_from_directory, session, redirect, url_for, render_template, abort

# Configurar logging para console (devido a problemas com arquivo)
logging.basicConfig(
    level=logging.INFO,
...
try:
    ...
    print("⚠️ Flask-Limiter não instalado - rate limiting desabilitado")
...
# Desabilitar SocketIO temporariamente devido a problemas de dependências
HAS_SOCKETIO = False
print("⚠️ SocketIO desabilitado - funcionalidades de chat limitadas")

# Criar mocks das funções SocketIO
class MockSocketIO:
...
app.config['RATELIMIT_STORAGE_URI'] = os.environ.get('IPPEL_RATE_LIMIT_STORAGE', 'memory://')
app.config['RATELIMIT_STORAGE_URL'] = app.config['RATELIMIT_STORAGE_URI']

# Rate limiting global por IP (fallback no-op se não instalado)
if HAS_LIMITER and Limiter is not None:
    _rate_storage = app.config.get('RATELIMIT_STORAGE_URI', 'memory://')
    limiter = Limiter(
        get_remote_address,
        app=app,
        storage_uri=_rate_storage,
        default_limits=["120 per minute"],  # ~2 rps/IP
    )
else:
...
# Security headers e CSP (sem forçar HTTPS, liberando mídia inline necessária)
allowed_script_src = [
    "'self'",
...
csp = {
    'default-src': ["'self'"],
    'script-src': allowed_script_src + ["'unsafe-inline'"],
    'style-src': allowed_style_src,
    'style-src-elem': allowed_style_src,
    'img-src': ["'self'", 'data:'],
...
# Permitir apenas hosts locais e da rede privada
def _build_allowed_hosts():
    ...
    try:
        ...
        pass
    try:
        hosts.add((socket.gethostbyname(socket.gethostname()) or '').lower())
    except Exception:
        pass
    return hosts

_ALLOWED_HOSTS = _build_allowed_hosts()
_PRIVATE_NET_RE = re.compile(r"^(10|127)\.|^192\.168\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.")

@app.before_request
def _enforce_host_and_methods():
    try:
        host = (request.host or '').split(':')[0].lower()
        if host and (host in _ALLOWED_HOSTS or _PRIVATE_NET_RE.match(host)):
            pass
        else:
            return abort(400)
        # Bloquear métodos inesperados globalmente
        if request.method not in {'GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'}:
            return abort(405)
    except Exception:
        return abort(400)

    # Fallback de sessão: se não há sessão mas existe cookie IPPEL_UID, reidrata a sessão
    try:
        if 'user_id' not in session:
            uid_cookie = request.cookies.get('IPPEL_UID')
            if uid_cookie:
                try:
                    uid = int(uid_cookie)
                except Exception:
                ...
                if uid:
                    ...
                    cur = conn.cursor()
                    cur.execute('SELECT id, name, email, department, role FROM users WHERE id = ? AND is_active = 1', (uid,))
                    row = cur.fetchone()
                    conn.close()
                    if row:
        ...
        # Não bloquear a requisição caso algo dê errado no fallback
        pass
...
# Servir arquivos estáticos de forma explícita (reforço) e fallback do logo
...
# Configurar SocketIO com fallback seguro (desabilitado temporariamente)
socketio = SocketIO(app)  # Mock instance
...
def init_database():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        group_permissions = {
            ...
            'Terceiros': [
                'view_own_rnc', 'chat_access', 'limited_access', 'view_respond_only'
            ],
            'Compras': [
    ...
    # Tabela de workflows de resposta
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS rnc_workflows (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            from_group_id INTEGER,
            to_group_id INTEGER,
            to_user_id INTEGER,
            workflow_type TEXT DEFAULT 'response',
            description TEXT,
            is_active BOOLEAN DEFAULT 1,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    # Inserir workflows padrão se não existirem
    cursor.execute('SELECT COUNT(*) FROM rnc_workflows')
    if cursor.fetchone()[0] == 0:
        # Workflows padrão: quando Terceiros/Expedição respondem, volta para Qualidade
        cursor.execute('SELECT id FROM groups WHERE name = "Terceiros"')
        terceiros_group = cursor.fetchone()
        cursor.execute('SELECT id FROM groups WHERE name = "Expedição"')
        expedicao_group = cursor.fetchone()
        cursor.execute('SELECT id FROM groups WHERE name = "Qualidade"')
        qualidade_group = cursor.fetchone()
        
        default_workflows = []
        if terceiros_group and qualidade_group:
            default_workflows.append((terceiros_group[0], qualidade_group[0], None, 'response', 'Terceiros -> Qualidade'))
        if expedicao_group and qualidade_group:
            default_workflows.append((expedicao_group[0], qualidade_group[0], None, 'response', 'Expedição -> Qualidade'))
        
        if default_workflows:
            cursor.executemany('''
                INSERT INTO rnc_workflows (from_group_id, to_group_id, to_user_id, workflow_type, description)
                VALUES (?, ?, ?, ?, ?)
            ''', default_workflows)
...
def share_rnc_with_user(rnc_id, shared_by_user_id, shared_with_user_id, permission_level='view'):
    """Compartilhar RNC com usuário com tolerância a lock do SQLite."""
    import time
    attempt = 0
    while attempt < 5:
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('BEGIN IMMEDIATE')  # força lock curto e previsível
            cursor.execute('''
                INSERT OR REPLACE INTO rnc_shares 
            ...
            if 'database is locked' in msg or 'database is busy' in msg:
                attempt += 1
                time.sleep(0.15 * attempt)  # backoff crescente
                continue
            logger.error(f"Erro ao compartilhar RNC: {e}")
            return False
        except Exception as e:
            logger.error(f"Erro ao compartilhar RNC: {e}")
            return False
...
def can_user_access_rnc(user_id, rnc_id):
    """Verificar se usuário pode acessar RNC (sempre permite acesso para todos os usuários)"""
    try:
        # Log para registrar a solicitação de acesso
        logger.info(f"Solicitação de acesso à RNC {rnc_id} pelo usuário {user_id} - ACESSO LIBERADO")
        
        # Verifique primeiro se a RNC existe para evitar erros em outras partes do código
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(*) FROM rncs WHERE id = ?', (rnc_id,))
        rnc_exists = cursor.fetchone()[0] > 0
        conn.close()
        ...
        # Sempre retorna True para permitir acesso irrestrito
        return True
        
    except Exception as e:
        logger.error(f"Erro ao verificar acesso à RNC: {e}")
        # Em caso de erro, também permite acesso para evitar bloqueios
        return True
...
def dashboard():
    ...
    user_permissions = {
        ...
        'isAdmin': session.get('user_role', '').lower() == 'admin' or has_permission(session['user_id'], 'admin_access'),
...
def get_user_info():
    ...
    try:
        ...
        department_permissions = {
            ...
            'viewRespondOnly': has_permission(session['user_id'], 'view_respond_only')  # Nova permissão
        }
...
def create_rnc():
    ...
    try:
        ...
        if signature_columns & cols:
            assinaturas = [data.get('assinatura1', ''), data.get('assinatura2', ''), data.get('assinatura3', '')]
        ...
        values_by_col = {
            ...
            # opcionais
            'signature_inspection_name': data.get('assinatura1', ''),
            'signature_engineering_name': data.get('assinatura2', ''),
            'signature_inspection2_name': data.get('assinatura3', ''),
    ...
    try:
        ...
        base_query = '''
                SELECT 
                    ...
                    r.department AS rnc_department
                FROM rncs r 
                LEFT JOIN users u ON r.user_id = u.id 
                LEFT JOIN users au ON r.assigned_user_id = au.id
        '''
        ...
        if tab == 'active':
            # RNCs ativos (não deletados, não finalizados)
            if has_permission(session['user_id'], 'view_all_rncs'):
                ...
                # Incluir RNCs criados pelo usuário E RNCs compartilhadas com ele
                cursor.execute(base_query + '''
                    WHERE r.is_deleted = 0 AND r.status != 'Finalizado' 
                    AND (r.user_id = ? OR r.id IN (
            ...
            # RNCs finalizados - aplicar regras de departamento
            if has_permission(session['user_id'], 'view_finalized_rncs'):
                # Qualidade, Administração e TI podem ver todos os finalizados
                cursor.execute(base_query + '''
                    WHERE r.is_deleted = 0 AND r.status = 'Finalizado'
                    ORDER BY r.finalized_at DESC
                ''')
            else:
                # Engenharia e Produção veem apenas RNCs criadas por eles
                cursor.execute(base_query + '''
                    WHERE r.is_deleted = 0 AND r.status = 'Finalizado' AND r.user_id = ?
                    ORDER BY r.finalized_at DESC
                ''', (session['user_id'],))

        else:
            # Aba inválida: padronizar para 'active'
            cursor.execute(base_query + '''
                WHERE r.is_deleted = 0 AND r.status != 'Finalizado'
                ORDER BY r.created_at DESC
            ''')
        ...
        for rnc in rncs:
            formatted_rncs.append({
                ...
                'user_department': rnc[17] or 'N/A',
                'department': rnc[18] or rnc[17] or 'N/A',  # Use rnc_department ou user_department como fallback
                'setor': rnc[18] or rnc[17] or 'N/A',  # Alias para compatibilidade
...
def view_rnc(rnc_id):
    ...
    try:
        ...
        return render_template('view_rnc.html', rnc=rnc_dict, is_creator=is_creator, txt_fields=txt_fields)
...
def update_rnc_api(rnc_id):
    ...
    try:
        ...
        if not (can_edit_all or (can_edit_own and user_is_creator) or has_admin or department_match):
        ...
        # Trancar assinaturas já existentes: não permitir sobrescrever nomes
        ...
        new_sign = (
            current_sign[0] or data.get('assinatura1', ''),
            current_sign[1] or data.get('assinatura2', ''),
            current_sign[2] or data.get('assinatura3', '')
        )
...
# Rota de restauração removida (sistema de lixeira desativado)

# Rota de exclusão permanente removida (não há lixeira)

# Rota de limpeza de lixeira removida
...
@app.route('/api/rnc/<int:rnc_id>/respond', methods=['POST'])
@limiter.limit("30 per minute")
def respond_to_rnc(rnc_id):
    """API para responder a uma RNC e automaticamente transferir conforme workflow"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Usuário não autenticado'}), 401
    ...
    try:
        ...
        # Verificar se o usuário pode acessar esta RNC
        if not can_user_access_rnc(session['user_id'], rnc_id):
            conn.close()
            return jsonify({'success': False, 'message': 'Acesso negado a esta RNC'}), 403
        
        # Buscar informações da RNC e do usuário atual
        cursor.execute('''
            SELECT r.assigned_user_id, r.user_id, u.group_id 
            FROM rncs r
            LEFT JOIN users u ON u.id = ?
            WHERE r.id = ?
        ''', (session['user_id'], rnc_id))
        ...
        current_assigned_user, original_creator, user_group_id = rnc_info
        
        # Adicionar mensagem ao chat
        cursor.execute('''
            INSERT INTO chat_messages (rnc_id, user_id, message, message_type)
            VALUES (?, ?, ?, 'response')
        ''', (rnc_id, session['user_id'], message))
        
        # Buscar workflow de resposta para este grupo
        new_assigned_user = None
        workflow_description = None
        
        if user_group_id:
            cursor.execute('''
                SELECT to_user_id, to_group_id, description
                FROM rnc_workflows 
                WHERE from_group_id = ? AND workflow_type = 'response' AND is_active = 1
                LIMIT 1
            ''', (user_group_id,))
            workflow = cursor.fetchone()
            
            if workflow:
                to_user_id, to_group_id, workflow_description = workflow
                
                if to_user_id:
                    # Workflow específico para um usuário
                    new_assigned_user = to_user_id
                elif to_group_id:
                    # Workflow para um grupo - selecionar usuário ativo do grupo
                    cursor.execute('''
                        SELECT id FROM users 
                        WHERE group_id = ? AND is_active = 1 AND role != 'admin'
                        ORDER BY id LIMIT 1
                    ''', (to_group_id,))
                    group_user = cursor.fetchone()
                    if group_user:
                        new_assigned_user = group_user[0]
        
        # Se não há workflow, voltar para o criador original
        if not new_assigned_user:
            new_assigned_user = original_creator
            workflow_description = "Retorno para criador original"
        
        # Atualizar assigned_user_id da RNC
        cursor.execute('''
            UPDATE rncs SET assigned_user_id = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        ''', (new_assigned_user, rnc_id))
        
        # Adicionar notificação automática sobre a transferência
        if new_assigned_user and new_assigned_user != session['user_id']:
            cursor.execute('''
                INSERT INTO notifications (user_id, rnc_id, type, title, message)
                VALUES (?, ?, 'response_transfer', 'RNC Respondida', ?)
            ''', (new_assigned_user, rnc_id, f'RNC foi respondida e transferida para você. {workflow_description or ""}'))
        
        conn.commit()
        conn.close()
        
        logger.info(f"RNC {rnc_id} respondida pelo usuário {session['user_id']} e transferida para {new_assigned_user}")
        
        return jsonify({
            'success': True,
            'message': 'Resposta enviada e RNC transferida com sucesso!',
            'transferred_to': new_assigned_user,
...
def get_workflows():
    """API para listar workflows de resposta"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Usuário não autenticado'}), 401
    
    if not has_permission(session['user_id'], 'admin_access'):
        return jsonify({'success': False, 'message': 'Sem permissão administrativa'}), 403
    ...
    try:
        ...
        cursor.execute('''
            SELECT w.id, w.workflow_type, w.description, w.is_active,
                   fg.name as from_group_name, tg.name as to_group_name, 
                   tu.name as to_user_name, tu.email as to_user_email
            FROM rnc_workflows w
            LEFT JOIN groups fg ON w.from_group_id = fg.id
            LEFT JOIN groups tg ON w.to_group_id = tg.id
            LEFT JOIN users tu ON w.to_user_id = tu.id
            ORDER BY fg.name, w.workflow_type
        ''')
        ...
        logger.error(f"Erro ao buscar workflows: {e}")
        return jsonify({'success': False, 'message': 'Erro interno do servidor'}), 500
...
def create_workflow():
    ...
    try:
        ...
        workflow_type = data.get('workflow_type', 'response')
        description = data.get('description', '')
        
        if not from_group_id:
            return jsonify({'success': False, 'message': 'Grupo de origem é obrigatório'}), 400
        
        if not to_group_id and not to_user_id:
            return jsonify({'success': False, 'message': 'Destino (grupo ou usuário) é obrigatório'}), 400
        ...
        cursor.execute('''
            INSERT INTO rnc_workflows (from_group_id, to_group_id, to_user_id, workflow_type, description)
            VALUES (?, ?, ?, ?, ?)
        ''', (from_group_id, to_group_id, to_user_id, workflow_type, description))
        
        conn.commit()
        conn.close()
        
        return jsonify({'success': True, 'message': 'Workflow criado com sucesso'})
...
def update_workflow(workflow_id):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE rnc_workflows 
            SET is_active = ?, description = ?
            WHERE id = ?
        ''', (is_active, description, workflow_id))
        
        conn.commit()
        conn.close()
        
        return jsonify({'success': True, 'message': 'Workflow atualizado com sucesso'})
...
@limiter.limit("60 per minute")
def get_chart_data():
    """API para fornecer dados dos gráficos"""
    if 'user_id' not in session:
        return jsonify({'error': 'Usuário não autenticado'}), 401
    
    # Verificar permissão para ver gráficos
...
@limiter.limit("60 per minute")
def get_enhanced_charts_data():
    """API avançada de dados para os gráficos da aba 'Gráficos'.
    Retorna KPIs, tendência, status, prioridade, departamentos (com eficiência)
    e matriz para heatmap. Baseado na tabela 'rncs'.
    """
    # Permitir acesso em modo debug/desenvolvimento
    ...
    # Verificar permissão para ver gráficos
    if not debug_mode and not has_permission(session['user_id'], 'view_charts'):
        return jsonify({'success': False, 'message': 'Acesso negado: usuário não tem permissão para visualizar gráficos'}), 403
    
    try:
        period = request.args.get('period', '30', type=int)
        cache_key = f"charts_enhanced_{session['user_id']}_{period}"
        cached = get_cached_query(cache_key)
        if cached:
            return jsonify(cached)
        ...
        cur.execute('''
            SELECT 
                COUNT(*) as total,
                SUM(CASE WHEN status IN ('Pendente','Em Análise','Em Andamento') THEN 1 ELSE 0 END) as pending,
                SUM(CASE WHEN status = 'Finalizado' THEN 1 ELSE 0 END) as resolved,
                SUM(CASE WHEN priority = 'Crítica' THEN 1 ELSE 0 END) as critical,
                AVG(CASE WHEN status = 'Finalizado' AND finalized_at IS NOT NULL 
                    THEN julianday(finalized_at) - julianday(created_at) 
                    ELSE NULL END) as avg_resolution_time
            FROM rncs
            WHERE created_at >= ? AND is_deleted = 0
        ''', (limit_date_str,))
        kpi_row = cur.fetchone() or (0,0,0,0,0)
        ...
        # Departamentos (com eficiência baseada no tempo médio de resolução)
        cur.execute('''
            SELECT COALESCE(u.department, 'Não Informado') as dept,
                   COUNT(*) as count,
                   AVG(CASE WHEN r.status = 'Finalizado' AND r.finalized_at IS NOT NULL
                        THEN julianday(r.finalized_at) - julianday(r.created_at)
                        ELSE NULL END) * 100 as efficiency
            FROM rncs r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE r.created_at >= ? AND r.is_deleted = 0
            GROUP BY dept
        ...
        payload = {
            ...
            'data': {
                'kpis': {
                    ...
                    'pending': kpi_row[1] or 0,
                    'resolved': kpi_row[2] or 0,
                    'critical': kpi_row[3] or 0,
                    'avgResolutionTime': round(kpi_row[4] or 0, 2)
                },
                'trend': [{'date': r[0], 'count': r[1], 'resolved': r[2], 'pending': r[3]} for r in trend_rows],
                'status': [{'label': r[0] or 'Não Informado', 'count': r[1], 'color': status_colors.get(r[0] or '', '#6c757d')} for r in status_rows],
                'priority': [{'label': r[0] or 'Não Informado', 'count': r[1], 'color': priority_colors.get(r[0] or '', '#6c757d')} for r in priority_rows],
                'departments': [{'label': r[0], 'count': r[1], 'efficiency': max(0, min(100, 100 - (r[2] or 5)))} for r in dept_rows],
                'heatmap': [{'y': r[0], 'x': r[1], 'v': r[2]} for r in heat_rows]
            }
        }
        cache_query(cache_key, payload, ttl=60)
        return jsonify(payload)
    except Exception as e:
        logger.error(f"Erro na API de gráficos avançados: {e}")
        try:
            return_db_connection(conn)
        except Exception:
            pass
...
@app.route('/api/levantamentos/ano')
def levantamento_ano():
    """Agrega valores mensais por departamento para RNCs finalizadas em um ano específico.
    Retorna header e rows com: Data (MM/YY), departamentos e Total.
    """
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Usuário não autenticado'}), 401
    try:
        ...
        if not year:
            return jsonify({'success': False, 'message': 'Parâmetro year é obrigatório'}), 400

        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT strftime('%m/%y', r.finalized_at) as my,
                   COALESCE(u.department, 'Outros') as dept,
                   SUM(COALESCE(r.price,0)) as total
            FROM rncs r
            JOIN users u ON r.user_id = u.id
            WHERE r.status = 'Finalizado'
              AND r.finalized_at IS NOT NULL
              AND strftime('%Y', r.finalized_at) = ?
              AND r.is_deleted = 0
            GROUP BY my, dept
            ORDER BY my
        ''', (str(year),))
        ...
        depts = ['Produção','Engenharia','Terceiros','Compras','Comercial','PCP','Expedição','Qualidade','Transporte']
        # Inicializar todos os meses do ano com zero
        my_to_totals = {f"{m:02d}/{str(year)[-2:]}": {d: 0.0 for d in depts + ['Outros']} for m in range(1,13)}
        for my, dept, total in rows:
            cat = dept if dept in depts else 'Outros'
            if my in my_to_totals:
                my_to_totals[my][cat] += float(total or 0)

        ordered_keys = [f"{m:02d}/{str(year)[-2:]}" for m in range(1,13)]
        data = []
        for k in ordered_keys:
            row = {'Data': k}
            total_mes = 0.0
            for d in depts:
                v = round(my_to_totals[k].get(d, 0.0), 2)
                row[d] = v
                total_mes += v
            row['Total'] = round(total_mes, 2)
            data.append(row)

        return jsonify({'success': True, 'year': year, 'header': ['Data'] + depts + ['Total'], 'rows': data})
    except Exception as e:
        logger.error(f"Erro no levantamento por ano: {e}")
        return jsonify({'success': False, 'message': 'Erro ao gerar levantamento por ano'}), 500

@app.route('/api/levantamentos/1415')
def levantamento_1415():
    """Agrega valores mensais por departamento para RNCs finalizadas em 2014 e 2015.
    Usa a coluna price (REAL) e o campo department do autor do RNC.
    """
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Usuário não autenticado'}), 401
    try:
        ...
        cursor.execute('''
            ...
            WHERE r.status = 'Finalizado'
              AND r.finalized_at IS NOT NULL
              AND (strftime('%Y', r.finalized_at) = '2014' OR strftime('%Y', r.finalized_at) = '2015')
              AND r.is_deleted = 0
            GROUP BY my, dept
            ORDER BY my
        ''')
        ...
        for my, dept, total in rows:
            cat = dept if dept in depts else 'Outros'
            if my not in my_to_totals:
                my_to_totals[my] = {d: 0.0 for d in depts + ['Outros']}
            my_to_totals[my][cat] += float(total or 0)

        def _sort_key(k):
            m, y = k.split('/')  # MM/YY
            return (int('20'+y), int(m))
        ordered_keys = sorted(my_to_totals.keys(), key=_sort_key)
        data = []
        for k in ordered_keys:
            row = {'Data': k}
            total_mes = 0.0
        ...
        return jsonify({'success': True, 'header': ['Data'] + depts + ['Total'], 'rows': data})
    except Exception as e:
        logger.error(f"Erro no levantamento 14-15: {e}")
        return jsonify({'success': False, 'message': 'Erro ao gerar levantamento'}), 500
...
def get_all_permissions():
    ...
    permissions = [
        ...
        {
            'name': 'delete_own_rnc',
            'display_name': 'Excluir Próprias RNCs',
            'description': 'Permite excluir RNCs criadas pelo próprio usuário'
        },
        {
            'name': 'view_all_rncs',
        ...
        {
            'name': 'edit_rncs',
            'display_name': 'Editar RNCs Gerais',
            'description': 'Permissão geral para edição de RNCs'
        },
        {
            'name': 'finalize_rnc',
            'display_name': 'Finalizar RNC',
            'description': 'Permite finalizar RNCs no sistema'
        },
        {
            'name': 'assign_rnc',
            'display_name': 'Atribuir RNC',
            'description': 'Permite atribuir RNCs a outros usuários'
        },
        {
            'name': 'chat_access',
            'display_name': 'Acesso ao Chat',
            'description': 'Permite acessar funcionalidades de chat'
        },
        {
            'name': 'technical_analysis',
            'display_name': 'Análise Técnica',
            'description': 'Permite realizar análises técnicas de RNCs'
        },
        {
            'name': 'limited_access',
            'display_name': 'Acesso Limitado',
            'description': 'Acesso limitado ao sistema'
        },
        {
            'name': 'purchase_analysis',
            'display_name': 'Análise de Compras',
            'description': 'Permite análise relacionada a compras'
        },
        {
            'name': 'commercial_analysis',
            'display_name': 'Análise Comercial',
            'description': 'Permite análise comercial de produtos/serviços'
        },
        {
            'name': 'planning_control',
            'display_name': 'Controle de Planejamento',
            'description': 'Permite controlar aspectos de planejamento'
        },
        {
            'name': 'shipping_access',
            'display_name': 'Acesso à Expedição',
            'description': 'Permite acessar funcionalidades de expedição'
        },
        {
            'name': 'quality_control',
            'display_name': 'Controle de Qualidade',
            'description': 'Permite acessar controles de qualidade'
        },
        {
            'name': 'admin_access',
        ...
        {
            'name': 'view_clients_admin',
            'display_name': 'Administrar Clientes',
            'description': 'Permite visualizar e gerenciar clientes'
        },
        {
            'name': 'view_operators_admin',
            'display_name': 'Administrar Operadores',
            'description': 'Permite visualizar e gerenciar operadores'
        },
        {
            'name': 'manage_users',
        ...
        },
        {
            ...
            'description': 'Permite visualizar RNCs que já foram finalizadas'
        },
        {
            'name': 'view_charts',
            'display_name': 'Visualizar Gráficos',
            'description': 'Permite acessar dashboards e gráficos'
        },
        {
            'name': 'view_reports',
            'display_name': 'Visualizar Relatórios',
            'description': 'Permite gerar e visualizar relatórios'
        },
        {
            'name': 'view_groups_for_assignment',
            'display_name': 'Visualizar Grupos para Atribuição',
            'description': 'Permite ver grupos disponíveis para atribuir RNCs'
        },
        {
            'name': 'view_users_for_assignment',
            'display_name': 'Visualizar Usuários para Atribuição',
            'description': 'Permite ver usuários disponíveis para atribuir RNCs'
        },
        {
            'name': 'view_all_departments_rncs',
            'display_name': 'Visualizar RNCs de Todos Departamentos',
            'description': 'Permite visualizar RNCs atribuídas para qualquer departamento'
        },
        {
            'name': 'view_engineering_rncs',
            'display_name': 'Visualizar RNCs da Engenharia',
            'description': 'Permite visualizar RNCs atribuídas para o departamento de Engenharia'
        }
    ]
    return permissions
...
def serve_test_page():
    ...
    try:
        ...
        return "<h1>Arquivo de teste não encontrado</h1>", 404

@app.route('/api/test/charts-data')
def test_charts_data():
    """API de teste para verificar consulta de dados"""
    ...
    try:
        ...
        result = {
            ...
            'status': [{'label': r[0], 'count': r[1]} for r in status_rows],
...
# Funções para filtrar informações baseadas em permissões de grupo
def filter_data_by_permissions(user_id, data_type, data):
    """Filtrar dados baseados nas permissões do grupo do usuário"""
    try:
        ...
        # Filtrar dados baseado no tipo e permissões
        if data_type == 'rncs':
            logger.info(f"Filtrando RNCs por permissões para usuário {user_id}. Total antes: {len(data)}")
            filtered = filter_rncs_by_permissions(user_id, user_permissions, data)
            logger.info(f"RNCs filtradas: {len(filtered)} de {len(data)}")
            return filtered
        elif data_type == 'charts':
            return filter_charts_by_permissions(user_permissions, data)
        elif data_type == 'reports':
            return filter_reports_by_permissions(user_permissions, data)
        elif data_type == 'users':
            return filter_users_by_permissions(user_permissions, data)
        else:
            return data
            
    except Exception as e:
        logger.error(f"Erro ao filtrar dados por permissões: {e}")
        return data

def filter_rncs_by_permissions(user_id, permissions, rncs):
    """Filtrar RNCs baseado nas permissões"""
    # Se usuário tiver admin_access garantir visão total
    if 'admin_access' in permissions:
        return rncs
    
    # Adicionar logs para diagnóstico
    logger.info(f"Filtrando RNCs para usuário {user_id} com permissões: {permissions}")
    
    filtered_rncs = []
    
    for rnc in rncs:
        # Log para diagnóstico
        rnc_info = f"RNC ID: {rnc.get('id')}, Departamento: {rnc.get('department')}, User_ID: {rnc.get('user_id')}"
        
        # Se pode ver todos os RNCs (incluindo permissão para todos departamentos)
        if 'view_all_rncs' in permissions or 'view_all_departments_rncs' in permissions:
            logger.debug(f"Incluindo por view_all_rncs/view_all_departments_rncs: {rnc_info}")
            filtered_rncs.append(rnc)
            continue
            
        # Se pode ver RNCs da Engenharia
        if 'view_engineering_rncs' in permissions and rnc.get('department') == 'Engenharia':
            logger.debug(f"Incluindo por view_engineering_rncs: {rnc_info}")
            filtered_rncs.append(rnc)
            continue
        
        # Se pode ver RNCs próprios
        if 'view_own_rnc' in permissions and str(rnc.get('user_id')) == str(user_id):
            logger.debug(f"Incluindo por view_own_rnc: {rnc_info}")
            filtered_rncs.append(rnc)
            continue
        
        # Outras permissões específicas por departamento (backup para compatibilidade)
        if 'view_technical_data' in permissions and rnc.get('department') == 'Engenharia':
            logger.debug(f"Incluindo por view_technical_data: {rnc_info}")
            filtered_rncs.append(rnc)
            continue
            
        if 'view_production_data' in permissions and rnc.get('department') == 'Produção':
            logger.debug(f"Incluindo por view_production_data: {rnc_info}")
            filtered_rncs.append(rnc)
            continue
    
    logger.info(f"Total de RNCs filtradas: {len(filtered_rncs)} de {len(rncs)}")
    return filtered_rncs

def filter_charts_by_permissions(permissions, charts_data):
    """Filtrar dados de gráficos baseado nas permissões"""
    if 'view_charts' not in permissions:
        # Retornar dados vazios se não tem permissão
        return {
            'total_rncs': 0,
            'por_status': [],
            'por_prioridade': [],
...
def filter_reports_by_permissions(permissions, reports_data):
    ...
    # Filtrar relatórios específicos baseado nas permissões
    if 'view_engineering_reports' in permissions:
        filtered_reports['engineering'] = reports_data.get('engineering', {})
    
    if 'view_admin_reports' in permissions:
        filtered_reports['admin'] = reports_data.get('admin', {})
    
    if 'quality_control' in permissions:
        filtered_reports['quality'] = reports_data.get('quality', {})
    
    return filtered_reports or reports_data

def filter_users_by_permissions(permissions, users_data):
    """Filtrar usuários baseado nas permissões"""
    if 'manage_users' not in permissions and 'admin_access' not in permissions:
        # Usuários sem permissão de gerenciar só veem informações básicas
        filtered_users = []
        for user in users_data:
            filtered_users.append({
                ...
                # Remover informações sensíveis como permissões, etc.
            })
...
# API para obter permissões específicas do usuário
@app.route('/api/user/group-permissions')
def get_user_group_permissions_api():
    """API para obter permissões específicas do grupo do usuário"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Usuário não autenticado'}), 401
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'permissions': permissions,
            'has_admin_access': 'admin_access' in permissions or 'all' in permissions,
            'can_view_all_rncs': 'view_all_rncs' in permissions or 'all' in permissions,
            'can_view_charts': 'view_charts' in permissions or 'all' in permissions,
            'can_manage_users': 'manage_users' in permissions or 'all' in permissions
        })
        
    except Exception as e:
        logger.error(f"Erro ao obter permissões do grupo: {e}")
        return jsonify({'success': False, 'message': 'Erro interno do servidor'}), 500
