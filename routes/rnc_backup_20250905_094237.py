...
def create_rnc():
    ...
    try:
        ...
        # Verificar assinaturas apenas se as colunas existirem
        signature_columns = {
            'signature_inspection_name',
            'signature_engineering_name',
            'signature_inspection2_name'
        }
        existing_signature_cols = signature_columns & cols
        if existing_signature_cols:
            ...
            # Verificar se pelo menos uma assinatura foi preenchida (não vazia e não igual a 'NOME')
            if not any(a and a.strip() and a.strip() != 'NOME' for a in assinaturas):
        ...
        # Validar assigned_user_id para evitar erro de chave estrangeira
        assigned_user_id = data.get('assigned_user_id')
        if assigned_user_id:
            cursor.execute('SELECT id FROM users WHERE id = ?', (assigned_user_id,))
            if cursor.fetchone() is None:
                assigned_user_id = None  # Usuário não encontrado, define como nulo
        else:
            assigned_user_id = None # Garante que seja nulo se não for fornecido
